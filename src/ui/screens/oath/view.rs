...
use crate::ui::screens::oath::view_model::{OathCredential, OathType, OathViewModel};
use crate::t;
...
impl OathViewModel {
    fn render_credential_row(&self, cred: &OathCredential, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            .id(SharedString::from(format!("oath-{}", cred.name)))
            .border_1()
                ...
                h_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(
                                Button::new(SharedString::from(format!("oath-del-{}", cred.name)))
                                    .ghost()
                                    .small()
                                    .child(
                                        Icon::default()
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let creds_element: AnyElement = if has_creds {
            ...
            v_flex()
                ...
                .child(div().text_lg().font_semibold().child(t!("oath-no-credentials")))
                .child(
                    div()
                        .text_color(theme.muted_foreground.clone())
                        .text_sm()
                        .text_center()
                        .max_w(px(384.0))
                        .child(t!("oath-no-credentials-desc")),
                )
                .into_any_element()
        };
...
impl OathViewModel {
    fn open_add_dialog(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let name_input = cx.new(|cx| {
            InputState::new(window, cx).placeholder("e.g. Google:user@gmail.com")
        });
        ...
        window.open_dialog(cx, move |dialog, _, _| {
            ...
            dialog
                ...
                .on_ok(move |_, _, cx| {
                    ...
                    let _ = entity_clone.update(cx, |this: &mut OathViewModel, cx| {
                        this.add_credential(OathCredential {
                            name,
                            secret,
                            cred_type: OathType::Totp,
                            digits: 6,
                            period: 30,
                            counter: 0,
        ...
        window.open_dialog(cx, move |dialog, window, cx| {
            ...
            dialog
                ...
                .on_ok(move |_, _, cx| {
                    ...
                    if let Some(uri) = uri {
                        let _ = entity_for_ok.update(cx, |this: &mut OathViewModel, cx| {
                            if let Some(cred) = OathViewModel::parse_otpauth_uri(&uri) {
